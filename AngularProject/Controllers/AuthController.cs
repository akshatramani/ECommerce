...
namespace AngularProject.Controllers
{
    ...
    public class AuthController : BaseController
    {
        ...
        public ApiResponseViewModel<Users> Register(Users objUser)
        {
            try
            {
                if (string.IsNullOrEmpty(objUser.Username) || !Regex.IsMatch(objUser.Username, @"\A(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)\Z", RegexOptions.IgnoreCase))
                {
                    return GenerateResponse.CreateResponse<Users>((int)HttpStatusCode.BadRequest, "Username(Email) is not in correct format", null, null);
                }
                ...
                ApiResponseViewModel<string> addUser = AddUser(objUser);
                ...
                return GenerateResponse.CreateResponse<Users>(addUser.StatusCode, addUser.Message, objUser, null);
            }
            catch (Exception ex)
            {
                return GenerateResponse.CreateResponse<Users>((int)HttpStatusCode.InternalServerError, ex.Message, null, null);
